...
class Tetris(object):
	...
	def remove_row(self, row: int) -> list:
		del self.board[row]
		result = [[0 for _ in range(self._config['cols'])]] + self.board

		self.rows_cleared += 1
		print(f"Rows cleared: {self.rows_cleared}")

		if self.rows_cleared % 5 == 0:
			self._config['delay'] = max(10, int(self._config['delay'] * 0.9))
			pygame.time.set_timer(pygame.USEREVENT+1, self._config['delay'])
		return result
	...
	def run(self) -> None:
		...
		while 1:
			...
			if self.gameover:
				self.center_msg(f"Game Over!\nPress space to continue\nFinal score: {self.rows_cleared}")
			else:
				if self.paused:
					self.center_msg("Paused\nCurrent score: " + str(self.rows_cleared))
				else:
	...
	def easy_mode(self):
		self.ez_mode = not self.ez_mode

		if self.ez_mode:
			self.shapes_backup = CONFIG['shapes']
			CONFIG['shapes'] = [ [[9,9,9,9,9,9,9,9]] ]
			print("EZ MODE ENABLED")
		else:
			CONFIG['shapes'] = self.shapes_backup
			print("EZ MODE DISABLED")
